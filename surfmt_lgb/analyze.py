"""SurfMT-LightGBM tree-model professional analysis entry point.

Trains the per-task LightGBM models (retraining at analysis time — not
persisted), then produces a full interpretability bundle under
...
Health note on tree-scale: LightGBM is scale-invariant on the feature side, so
standardization of the *target* does not affect any of this analysis — features
are used at their raw scale throughout.

Usage:
    python surfmt_lgb/analyze.py                        # all tasks
    python surfmt_lgb/analyze.py --task 0               # single task only
    python surfmt_lgb/analyze.py --output_dir outputs/lgb_analysis --seed 42
"""
import argparse
import json
import sys
...
# Allow running as `python surfmt_lgb/analyze.py`
sys.path.insert(0, str(Path(__file__).resolve().parent.parent))

from surfmt_lgb.data import load_data
from surfmt_lgb.train import train_all_tasks
from surfmt_lgb.metrics import compute_metrics, TASK_NAMES
from surfmt_lgb import interpret as itp


def _denorm(pred_std, scalers):
    """Invert per-task target standardization (+ optional log) to raw scale."""
    denorm = np.zeros_like(pred_std)
    for t, (transform, mean, std) in enumerate(scalers):
        y_trans = pred_std[:, t] * std + mean
        denorm[:, t] = np.exp(y_trans) if transform == "log" else y_trans
    return denorm


def main():
    parser = argparse.ArgumentParser(description="SurfMT-LightGBM tree analysis")
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--output_dir", type=str, default="outputs/lgb_analysis")
    ...
    models, logs, scalers = train_all_tasks(
        ...
        seed=args.seed, num_rounds=args.max_rounds,
    ...
    denorm = _denorm(pred_std, scalers)
    ...
    block_gain = {}
    block_perm = {}
    desc_gain_matrix = np.zeros((len(models), len(itp.DESC_COL_NAMES)))
    top_desc = {}
    for t in tasks:
        model = models[t]
    ...
    if args.no_shap:
        ...
        for t in tasks:
            ...
            top_desc[TASK_NAMES[t]] = [
                (itp.DESC_COL_NAMES[i], float(g_named[i] / itp.gain_importance(model).sum()),
    ...
    meta = {
        ...
        "n_features": int(n_features), "max_rounds": args.max_rounds,
        "early_stopping": args.early_stopping, "shap_samples": args.shap_samples,
