"""Tree-model interpretability helpers for SurfMT-LightGBM.

Provides the building blocks used by ``analyze.py``: feature-name mapping,
LightGBM native / block / permutation feature importance, SHAP
interpretability, Morgan bit -> substructure reverse mapping, plotting, and
report generation.

The model is a set of per-task LightGBM Boosters (see ``train.py``), each
trained on a 4329-dim tabular vector (ECFP4 + ECFP6 + MACCS + 65 descriptors +
temperature). Only the 65 descriptor columns and the temperature column have
human-readable names; the ~4263 fingerprint/MACCS bits are labelled
...
def gain_importance(model):
    """Native LightGBM gain importance -> [4329]."""
    return np.asarray(model.feature_importance(importance_type="gain"), dtype=float)


def split_importance(model):
    """Native LightGBM split (frequency) importance -> [4329]."""
    return np.asarray(model.feature_importance(importance_type="split"), dtype=float)
...
def shap_values(model, X):
    """Tree SHAP values for a booster on ``X`` -> [N, 4329].

    ``TreeExplainer`` writes in C and is fast even on 4329 columns; callers
...
def write_report(out_path, meta):
    ...
    """
    lines = ["# SurfMT-LightGBM 树模型专业分析报告", ""]
    ...
    lines += ["- 每任务为独立 LightGBM 回归器，目标经 log 变换（如适用）后标准化；"
              "特征保持原始尺度。", ""]
    rows = [[TASK_NAMES[int(k.rsplit("_", 1)[1])], m["samples"],
             m["best_iteration"], m["transform"]]
            for k, m in meta["logs"].items()]
    lines += [_md_table(["任务", "样本数", "best_iter", "目标变换"], rows), ""]

    # 2. Block importance -----------------------------------------------------
    lines += ["## 2. 特征分块重要性（归一化 gain）", ""]
    ...
    lines += ["## 3. 每任务 top 描述符（gain 与 SHAP）", ""]
    ...
    lines += ["## 5. 关键指纹位 → 化学子结构", ""]
    lines += [f"top-{meta.get('top_bits', 20)} 个高重要性指纹位（按跨任务 gain 聚合）"
    ...
    # 6. Reproducibility ------------------------------------------------------
    lines += ["## 6. 可复现性", ""]
    lines += [f"- 随机种子：`{meta['seed']}`；`max_rounds={meta['max_rounds']}`；"
              f"`early_stopping={meta['early_stopping']}`；`shap_samples={meta['shap_samples']}`", ""]
    lines += ["- 训练与特征代码：`surfmt_lgb/{train,features,data}.py`（未改动，分析时重训）", ""]
